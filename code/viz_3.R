...
df_viz3 = read.csv("data\\consolidated\\final_rent.csv", header = T)
...
df_viz3_2 = merge(df_viz3_1,df_viz3) 

# select relevant columns
df_viz3_2 = (df_viz3_2 
             %>% select(Year, group_number_rooms, Rent_Prices, Median.Property.Value, Median.Household.Income))
...
plot_theme = (
  ggplot(df_viz3_2, aes(x=Median.Property.Value/1000)) +
    ...
    labs(
      x='Median Property Value in 1000 US Dollar',
      y='Median Rent Price in US Dollar',
      title = 'Rent Price vs. Property Value in Stillwater',
      subtitle = 'This chart shows the relationship between median rent prices and median property value in Stillwater.'
    ) +
    theme(
...
# determining dataframe for first regression
df_reg1 = df_viz3_2 %>% filter(group_number_rooms == '2 rooms or less') %>% mutate(Median.Property.Value=Median.Property.Value/1000)

# using lm - linear model to obtain model parameters
reg1<-lm(formula = Rent_Prices~Median.Property.Value, data=df_reg1)  
...
df_reg2 = df_viz3_2 %>% filter(group_number_rooms == '3 rooms or more') %>% mutate(Median.Property.Value=Median.Property.Value/1000)

reg2<-lm(formula = Rent_Prices~Median.Property.Value, data=df_reg2)  
...
plot_theme + 
  ...
  annotate('text', x = 175, y = 680, size = 4, color='red', 
           label = glue::glue('Slope = ', round(slope1[[1]],2))) + 
  annotate('text', x = 168, y = 1100, size = 4, color='blue', 
           label = glue::glue('Slope = ', round(slope2[[1]],2))) +
  geom_label(x = 168, y = 800, size = 4, color='black', fill = 'pink',
             label = glue::glue('When there are 2 rooms or less, there are virtually \nno relationship between rent prices and property values.')) + 
  # input arrow
  geom_curve(data = arrow_1, 
             ...
             color = 'red', 
             curvature = 0.2) +
  geom_label(x = 175, y = 1000, size = 4, color='black', fill = 'lightblue',
             label = glue::glue('When there are 3 rooms or more, rent prices increases with property value.')) + 
